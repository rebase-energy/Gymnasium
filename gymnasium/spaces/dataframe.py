...
#TODO 
# It will probably not be possible to use DataFrame as a space. 
# Better idea is to use Dict space and then convert it to DataFrame in the class. 
# Meaning you define your DataFrame space as: 
# 1) Dict space with keys as column names and values as spaces for each column
# 2) Index with time range, frequency, and/or set of possible dates
# Would be nice to have a "sequential sampling method" that keeps the state and continously samples the next state. 

class DataFrame(Space[], typing.Mapping[str, Space[Any]]):
